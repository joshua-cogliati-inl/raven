...
class DaskRunner(InternalRunner):
  ...
  def getReturnCode(self):
    ...
    if not self.hasBeenAdded:
      try:
        ...
        self.raiseAWarning(self.__class__.__name__ + " job "+self.identifier+" failed with error:"+ str(ae) +" !",'ExceptedError1')
        self.returnCode = -1
      ...
      if not self.hasBeenAdded:
        if self.__func is not None:
          ...
          try:
            ...
            self.returnCode = -1
            self.raiseAWarning(self.__class__.__name__ + " job "+self.identifier+" failed with error:"+ str(ae) +" !",'ExceptedError2')
            raise ae
        else:
          self.runReturn = None
        self.hasBeenAdded = True
  ...
  def start(self):
    ...
    try:
      ...
      self.raiseAWarning(self.__class__.__name__ + " job "+self.identifier+" failed with error:"+ str(ae) +" !",'ExceptedError3')
      self.returnCode = -1
